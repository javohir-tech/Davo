  ...
  <div class="medicines-container">
    ...
    <div v-else class="cards-wrapper">
      ...
      <div v-if="paginatedMedicines.length > 0" class="cards-grid">
        <div v-for="medicine in paginatedMedicines" :key="medicine.id" class="medicine-card">
          <!-- Card header with image -->
          <div class="card-header">
            <div class="medicine-icon">💊</div>
            <div :class="['stock-indicator', medicine.stock < 100 ? 'low-stock' : 'in-stock']">
              <span class="stock-dot"></span>
              {{ medicine.stock < 100 ? 'Kam qoldi' : 'Mavjud' }} </div>
            </div>
            ...
            <div class="card-body">
              ...
              <div class="price-section">
                ...
                <div class="price-value">{{ medicine.price.toLocaleString() }} <span class="currency">so'm</span></div>
              </div>
            </div>

            <!-- Card footer with actions -->
            <div class="card-footer">
              <Button type="default" block class="view-btn" @click="viewMedicine(medicine.id)">
                <EyeOutlined />
                Batafsil
              </Button>
              <Button type="primary" block class="cart-btn" @click="addToCart(medicine)">
        ...
        <div v-if="filteredMedicines.length > pageSize" class="pagination-wrapper">
          <a-pagination v-model:current="currentPage" :total="filteredMedicines.length" :page-size="pageSize"
            ...
            :show-total="(total, range) => `${range[0]}-${range[1]} / ${total} ta dori`" />
        </div>
      </div>
    </div>
</template>
...
//Antd Icons
import { SearchOutlined, ShoppingCartOutlined, EyeOutlined } from '@ant-design/icons-vue';
//Hooks
