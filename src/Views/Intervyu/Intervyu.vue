    ...
    <div class="contact-page">
        <div class="contact-container">
            ...
            <div class="form-wrapper">
                <a-form :model="formState" :rules="rules" :label-col="{ span: 24 }" :wrapper-col="{ span: 24 }"
                    @finish="onSubmit" ref="formRef" layout="vertical">
                    <!-- Ism -->
                    <a-form-item label="To'liq ism" name="fullName" has-feedback>
                        <a-input v-model:value="formState.fullName" placeholder="Ismingiz va familiyangizni kiriting"
                            size="large" :prefix="h(UserOutlined)" />
                    </a-form-item>

                    <!-- Yosh -->
                    <a-form-item label="Yoshingiz" name="age" has-feedback>
                        <a-input-number v-model:value="formState.age" placeholder="Yoshingizni kiriting" size="large"
                            :min="1" :max="120" style="width: 100%" />
                    </a-form-item>

                    <!-- Telefon -->
                    <a-form-item label="Telefon raqami" name="phone" has-feedback>
                        <a-input v-model:value="formState.phone" placeholder="+998 90 123 45 67" size="large"
                            :prefix="h(PhoneOutlined)" />
                    </a-form-item>

                    <!-- Kasallik tavsifi -->
                    <a-form-item label="Sizni bezovta qilayotgan muammo" name="description" has-feedback>
                        <a-textarea v-model:value="formState.description"
                            placeholder="Kasallik yoki muammo haqida qisqacha ma'lumot bering..." :rows="5" size="large"
            ...
            <div class="info-section">
                <a-alert type="info" show-icon closable>
                    ...
                    <template #description>
                        Ma'lumotlaringiz maxfiy saqlanadi va faqat shifokorlarimiz tomonidan ko'rib chiqiladi.
                        Biz 24 soat ichida siz bilan bog'lanamiz.
                    </template>
                </a-alert>
            </div>
        </div>
...
const rules = {
    ...
    phone: [
        ...
        {
            pattern: /^(\+998|998)?[\s\-]?[0-9]{2}[\s\-]?[0-9]{3}[\s\-]?[0-9]{2}[\s\-]?[0-9]{2}$/,
...
const onSubmit = async () => {
    try {
        ...
        message.success({
            content: 'Murojaatingiz muvaffaqiyatli yuborildi! Tez orada siz bilan bog\'lanamiz.',
            duration: 5
        });
        ...
        // Xatolik xabari
        message.error({
            content: 'Xatolik yuz berdi. Iltimos, qaytadan urinib ko\'ring yoki to\'g\'ridan-to\'g\'ri qo\'ng\'iroq qiling.',
            duration: 5
        });
    } finally {
        loading.value = false;
