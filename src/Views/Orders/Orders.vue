    ...
    <div class="orders-container">
        <div class="page-header">
            ...
            <a-space :size="[8, 8]" wrap class="header-actions">
                <a-select v-model:value="filterStatus" style="min-width: 140px" placeholder="Status"
                    @change="loadOrders">
        ...
        <a-spin :spinning="loading" size="large">
            <a-empty v-if="orders.length === 0 && !loading" description="Hali buyurtmalar yo'q"
            ...
            <div v-else class="orders-grid">
                <div v-for="item in paginatedOrders" :key="item.id" class="order-card-wrapper">
                    <div :class="['order-card', `status-${item.status || 'pending'}`]">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-text">{{ item.yetkazibBerishMalumotlari.manzil }}</span>
                        </div>

                        <!-- Phone -->
                        <div class="info-row">
                            <PhoneOutlined class="info-icon" />
                            <span class="info-text">{{ item.yetkazibBerishMalumotlari.telefonRaqami }}</span>
                        </div>

                        <!-- Delivery Time -->
                        <div class="info-row delivery-time">
                            <span class="info-label">Yetkazish:</span>
                            <span class="info-value">{{ item.yetkazibBerishMalumotlari.yetkazibBerishVaqti }}</span>
                        </div>
                        ...
                        <div class="drugs-section">
                            ...
                            <div class="drugs-list">
                                <div v-for="(drug, index) in item.dorilar" :key="index" class="drug-item">
                                    <div class="drug-main">
                        ...
                        <div v-if="item.shifokorKodi" class="doctor-code">
                            <UserOutlined />
                            <span>Shifokor: <strong>{{ item.shifokorKodi }}</strong></span>
                        </div>
                        ...
                        <div class="price-section">
                            ...
                            <div class="price-row total">
                                <span>Jami:</span>
                                <span class="total-price">{{ formatPrice(item.narxlar.umumiySumma) }}</span>
                            </div>
                        </div>

                        <!-- Actions -->
                        <div class="card-actions">
                            <a-button type="primary" ghost block @click="showOrderDetails(item)">
                            ...
                            <!-- delete modal -->
                            <a-modal v-model:open='deleteOrderVisible' title="Buyurtmani Bekor qilish"
            ...
            <div v-if="orders.length > 0" class="pagination-wrapper">
                <a-pagination v-model:current="currentPage" v-model:page-size="pageSize" :total="orders.length"
                    :show-size-changer="true" :show-total="(total) => `Jami: ${total} ta`"
                    :page-size-options="['6', '12', '24', '48']" @change="handlePageChange"
                    @showSizeChange="handlePageChange" />
            </div>
        </a-spin>

        <!-- Details Modal -->
        <a-modal v-model:open="detailsModalVisible" title="Buyurtma Tafsilotlari" width="800px" :footer="null" centered>
            <div v-if="selectedOrder" class="modal-content">
                <a-descriptions bordered :column="{ xs: 1, sm: 1, md: 2 }" size="middle">
                    <a-descriptions-item label="ID" :span="2">
                        <a-typography-text copyable>{{ selectedOrder.id }}</a-typography-text>
                    </a-descriptions-item>
                    ...
                    <a-descriptions-item v-if="selectedOrder.shifokorKodi" label="Shifokor kodi" :span="2">
                ...
                <a-table :columns="drugColumns" :data-source="selectedOrder.dorilar" :pagination="false" size="small"
                    :scroll="{ x: 500 }">
                    <template #bodyCell="{ column, record }">
                        ...
                        <template v-if="column.key === 'retseptKerak'">
                            <a-tag :color="record.retseptKerak ? 'orange' : 'green'" size="small">
                ...
                <div class="modal-summary">
                    ...
                    <div class="summary-row">
                        ...
                        <span>{{ formatPrice(selectedOrder.narxlar.yetkazibBerishNarxi) }}</span>
                    </div>
                    <div class="summary-row total">
                        <span>Umumiy summa:</span>
                        <span class="total-amount">{{ formatPrice(selectedOrder.narxlar.umumiySumma) }}</span>
                    </div>
                </div>
            </div>
        </a-modal>
...
const drugColumns = [
    ...
    { title: 'Soni', dataIndex: 'soni', key: 'soni', align: 'center', width: 80 },
    { title: 'Narxi', dataIndex: 'narxi', key: 'narxi', align: 'right', width: 120 },
    { title: 'Jami', key: 'jami', align: 'right', width: 120 },
    { title: 'Retsept', dataIndex: 'retseptKerak', key: 'retseptKerak', align: 'center', width: 100 }
];
...
const loadOrders = async () => {
    loading.value = true;
    try {
        const ordersQuery = query(collection(db, 'Orders'), orderBy('vaqt', 'desc'));
        ...
        if (filterStatus.value !== 'all') {
            orders.value = orders.value.filter(order =>
                (order.status || 'pending') === filterStatus.value
            );
        }
