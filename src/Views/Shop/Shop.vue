        ...
        <template #title>
          ...
          >
            <div class="header-content">
              ...
              <a-badge
                :count="drugStore.selectedCount"
                :number-style="{ backgroundColor: '#52c41a' }"
              />
            </div>
            <RouterLink to="/orders">
              <a-button type="primary"> Buyurtmalar </a-button>
            </RouterLink>
          </div>
        </template>
      </a-card>
      ...
      <a-row :gutter="[16, 16]">
        <a-col :xs="24" :lg="16">
          <a-card :bordered="false" class="cart-items">
            <div
              class="loading-selecteds"
              v-if="
                drugStore.loadingSelecteds &&
                drugStore.selectedDrugs.length === 0
              "
            >
              <a-spin size="large" />
            </div>
            ...
            >
              <RouterLink to="/drugs">
                <a-button type="primary"> Xaridni boshlash </a-button>
              </RouterLink>
            </a-empty>
            <div
              v-for="item in drugStore.selectedDrugs"
              :key="item.id"
              class="cart-item"
            >
              <a-row :gutter="16" align="middle">
                ...
                <a-col :xs="12" :sm="4" :md="4">
                  <div class="quantity-control">
                    <a-button
                      size="small"
                      @click="drugStore.decrementQuantity(item.id)"
                      :disabled="drugStore.getQuantity(item.id) <= 1"
                    >
                      <minus-outlined />
                    </a-button>
                    <span class="quantity">{{
                      drugStore.getQuantity(item.id)
                    }}</span>
                    <a-button
                      size="small"
                      @click="drugStore.incrementQuantity(item.id)"
                    >
                      <plus-outlined />
                    </a-button>
                  </div>
                ...
                <a-col :xs="12" :sm="4" :md="4">
                  <div class="item-actions">
                    <div class="item-price">
                      {{
                        formatPrice(drugStore.getQuantity(item.id) * item.price)
                      }}
                      so'm
                    </div>
                    <a-button
                      type="text"
                      danger
                      @click="drugStore.removeDrug(item.id)"
                      class="remove-btn"
                    >
          ...
          <a-button
            v-if="drugStore.selectedCount > 0"
            size="large"
            type="primary"
            danger
            block
        ...
        <a-col :xs="24" :lg="8">
          <a-card :bordered="false" class="summary-card">
            <template #title> <calculator-outlined /> Jami hisob </template>

            <div class="summary-row">
              <span>Mahsulotlar soni:</span>
              <span class="summary-value"
                >{{ drugStore.selectedCount }} ta</span
              >
            </div>
            ...
            <div class="summary-row">
              ...
              <span class="summary-value"
                >{{ formatPrice(totalPrice) }} so'm</span
              >
            </div>
            ...
            >
              ...
              <a-form layout="vertical">
                <!-- Manzil -->
                <a-form-item
                  label="Yetkazib berish manzili Hozirda faqat Toshkent shahriga buyutmalar qabul qilamiz"
                  required
                >
                  <a-input
                ...
                <a-form-item label="Yetkazib berish vaqti" required>
                  <a-select
                    ...
                    placeholder="Yetkazib berish vaqtini tanlang"
                    size="large"
                ...
                <a-form-item v-if="hasRxItems" label="Retsept kodi" required>
                  <a-input
                    v-model:value="doctorCode"
                    placeholder="Retseptli dorilar uchun shifokor kodini kiriting"
                    size="large"
                  />
                  <span style="color: #faad14; font-size: 12px">
                    ⚠️ Agar savatingizda retsept talab qilinadigan dorilar
                    bo'lsa, shifokorlarimiz sizga bergan kodni kiritishingiz
                    kerak.
                  </span>
                </a-form-item>
                ...
                <div class="order-items">
                  <div
                    v-for="drug in drugStore.selectedDrugs"
                    :key="drug.id"
                    class="order-item"
                  >
                    <div class="item-info">
                      <span class="item-name-modal">
                        {{ drug.name }}
                        <span
                          v-if="drug.prescriptionRequired"
                          class="prescription-badge"
                        >
                          Retsept
                        </span>
                      </span>
                      <span class="item-quantity-modal"
                        >Soni: {{ drug.quantity }}</span
                      >
                    </div>
                ...
                <div class="price-breakdown">
                  ...
                  <div class="price-row-modal total-row-modal">
                    ...
                    <span
                      ><strong>{{ formatPrice(total) }} so'm</strong></span
                    >
                  </div>
                </div>

                <div
                  v-if="subtotal < FREE_DELIVERY_THRESHOLD"
                  class="delivery-info"
                >
                  ℹ️ Bepul yetkazib berish uchun yana
                  {{ formatPrice(FREE_DELIVERY_THRESHOLD - subtotal) }} so'm
                  qo'shing
                </div>
              </a-form>
            </a-modal>
...
<script setup>
  ...
  const totalPrice = computed(() => {
    return drugStore.selectedDrugs.reduce(
  ...
  const isFormValid = computed(() => {
    const basicFieldsValid =
      address.value && phoneNumber.value && deliveryTime.value
    if (hasRxItems.value) {
      return (
        basicFieldsValid &&
        doctorCode.value === import.meta.env.VITE_LOCAL_RETSEPT_KEY
      )
    }
...
🕐 Vaqt: ${new Date().toLocaleString('uz-UZ')}
    `.trim()
      ...
      // Firebase ga saqlash
      await setDoc(
        doc(db, 'Orders', `${userStore.uid}_${new Date().getTime()}`),
        orderData
      )
      ...
      const telegramResponse = await fetch(TELEGRAM_BOT_URL, {
        ...
        body: JSON.stringify({
          // bodyody emas, body bo'lishi kerak!
