  ...
  actions: {
    async fetchSelectedDrugs() {
      ...
      try {
        ...
        this.selectedCount = data.value.reduce(
    ...
    async addDrug(drug) {
      ...
      try {
        ...
        if (userStore.isActive) {
          await setDoc(
            doc(db, 'selectedMedicines', this.userId, 'orders', drug.id),
            { ...drug, quantity: 1 }
          )
    ...
    async removeDrug(drugId) {
      ...
      try {
        ...
        if (drug) {
          await deleteDoc(
            doc(db, 'selectedMedicines', this.userId, 'orders', drugId)
          )
    ...
    async clearList() {
      ...
      const querySnapshot = await getDocs(
        collection(db, 'selectedMedicines', this.userId, 'orders')
      )
      const deletePromises = querySnapshot.docs.map((document) => {
        deleteDoc(
          doc(db, 'selectedMedicines', this.userId, 'orders', document.id)
        )
      })
